...
const DEFAULT_CONFIG: DBConfig = {
  ...
  password: '',
  useProxy: false,
  firebaseApiKey: "REMOVED",
  firebaseAuthDomain: "REMOVED",
  firebaseProjectId: "REMOVED",
  firebaseStorageBucket: "REMOVED.firebasestorage.app",
  firebaseMessagingSenderId: "REMOVED",
  firebaseAppId: "1:REMOVED:web:1f45b62ff0a49389caf2c9",
...
export const fetchDictionaryFromPostgres = async (): Promise<DictionaryField[]> => {
  ...
  if (config.type === 'firebase') {
   ...
   try {
      ...
      console.log('[DB] Loaded from Postgres:', data.length, 'fields');
      console.log('[DB] Sample:', data.slice(0, 3).map(d => ({ field_name: d.field_name, unit: d.unit, possible_values: d.possible_values?.substring(0, 30) })));
...
export const saveDictionaryToPostgres = async (dictionary: DictionaryField[]) => {
  ...
  try {
    ...
    console.log('[saveDictionaryToPostgres] payload sample:', payload.filter(p => p.unit || p.possible_values).slice(0, 3));
    console.log('[saveDictionaryToPostgres] total fields:', payload.length);
    console.log('[saveDictionaryToPostgres] fields with unit:', payload.filter(p => p.unit).length);
    console.log('[saveDictionaryToPostgres] fields with possible_values:', payload.filter(p => p.possible_values).length);
