...
import { CATEGORY_KTRU_INDICES } from '../constants';
...
const parseValueAndUnit = (fieldName: string, raw: string): { value: string | number, unit: string } => {
  ...
  const contrastMatch = cleaned.match(/^(\d{2,10})\s*:\s*1$/);
  if (contrastMatch || (fieldLower.includes('контрастность') && cleaned.match(/^\d{2,10}$/))) {
    const value = contrastMatch ? contrastMatch[1] : cleaned;
  ...
  try {
    ...
    orders.forEach(order => {
      ...
      (order.items || []).forEach(item => {
        if (item && item.name) allItemNames.add(item.name);
      });
    });
    ...
    sortedItemNames.forEach(itemName => {
      ...
      sortedDates.forEach(date => {
        ...
        const totalQty = ordersOnDate.reduce((sum, order) => {
          const itemsInOrder = (order.items || []).filter(i => i && i.name === itemName);
          const qtyInOrder = itemsInOrder.reduce((qSum, item) => qSum + (item.quantity || 0), 0);
    ...
    categories.forEach(cat => {
      ...
      orders.forEach(o => {
        if (!o || !o.items) return;
        o.items.filter(i => i && i.category === cat).forEach(item => {
          if (item) allItemsInCat.push({ item, metadata: o.metadata });
        });
      });
      ...
      const dictFields = dictionary
        .filter(f => f && f.category === cat && f.isActive)
        .map(f => f.fieldName);
      ...
      allItemsInCat.forEach(entry => {
        ...
        (entry.item.characteristics || []).forEach(char => {
          ...
          const isKnown = dictFields.some(df => df && df.toLowerCase() === char.name.toLowerCase());
      ...
      const orderedKnownFields = dictFields.filter(df =>
        df && Array.from(foundFields).some(f => f.toLowerCase() === df.toLowerCase())
      );
      ...
      const headers = ['Параметр \\ Устройство', 'Ед. изм.', ...allItemsInCat.map(e => {
        ...
        return `${e.item.name || 'Товар'} (${d})`;
      })];

      rows.push(headers);
      
      rows.push(['Код КТРУ', '', ...allItemsInCat.map(e => (e.item && e.item.ktruCode) || '—')]);
      ...
      rows.push(['ID обработки', '', ...allItemsInCat.map(e => e.metadata.processingId || '—')]);
      rows.push(['Файл-источник', '', ...allItemsInCat.map(e => e.metadata.sourceFile || '—')]);
      ...
      rows.push(['Количество (ед.)', '', ...allItemsInCat.map(e => (e.item && e.item.quantity) || 0)]);

      allFields.forEach(field => {
        const rawValues = allItemsInCat.map(entry => {
          if (!entry.item) return '';
          const char = (entry.item.characteristics || []).find(c => c && c.name && c.name.toLowerCase() === field.toLowerCase());
...
export const generateTemplate = async (
  ...
  try {
    ...
    Object.values(EquipmentCategory).forEach(cat => {
      const catFields = dictionary.filter(f => f.category === cat && f.isActive);
      const fields = catFields.map(f => f.fieldName);
      if (fields.length > 0 || cat === EquipmentCategory.OTHER) {
        const displayFields = fields.length > 0 ? fields : ['Описание'];
        const rows: any[][] = [
          ['Параметр \\ Устройство', 'Ед. изм.', 'Значения', 'Устройство 1'],
          ['Код КТРУ', CATEGORY_KTRU_INDICES[cat] || '', '', ''],
          ['ИНН заказчика', '', '', ''],
          ['Название заказчика', '', '', ''],
          ['Дата заказа', '', '', ''],
          ['Дата обработки', '', '', ''],
          ...
          ...displayFields.map(field => {
            const unitMap = unitsByCategory?.[cat] || {};
            let unit = unitMap[field] || '';
            if (!unit) {
              const key = normalizeKey(field);
              const matchKey = Object.keys(unitMap).find(k => {
                const nk = normalizeKey(k);
                return nk === key || nk.includes(key) || key.includes(nk);
              });
              if (matchKey) unit = unitMap[matchKey] || '';
            }
            const fieldData = catFields.find(f => f.fieldName === field);
        ...
        const ws = XLSX.utils.aoa_to_sheet(rows);
        ws['!cols'] = [{ wch: 40 }, { wch: 15 }, { wch: 30 }, { wch: 30 }];
        XLSX.utils.book_append_sheet(wb, ws, cat.substring(0, 31));
      }
    });
...
export const fillExcel = async (
  ...
  try {
    ...
    const summaryRows: any[][] = [
      ...
      ['Файл-источник', metadata.sourceFile || '—'],
    ...
    categoriesInOrder.forEach(cat => {
      ...
      const rows: any[][] = [
        ...
        ['ID обработки', '', ...catItems.map(() => metadata.processingId)],
        ['Файл-источник', '', ...catItems.map(() => metadata.sourceFile || '—')],
